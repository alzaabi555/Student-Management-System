...
const A4_HEIGHT_MM = 297; // ارتفاع A4 القياسي
const A4_WIDTH_PX = 794; 
const A4_HEIGHT_PX = 1123; 
const ROWS_PER_PAGE = 20; // عدد الطلاب في الصفحة الواحدة لضمان عدم انقصاص الجدول
...
const generateAndSharePDF = async (contentHTML: string, fileName: string) => {
    ...
    // لا نحدد Height هنا، نتركه ينمو حسب عدد الصفحات
    ...
    try {
        ...
        // نلتقط الصورة للطول الكامل (الذي قد يحتوي على عدة صفحات مرصوصة تحت بعضها)
        ...
        // إضافة الصفحات التالية
        while (heightLeft > 0) {
            position = heightLeft - imgHeight; // نسحب الصورة للأعلى
            pdf.addPage();
            pdf.addImage(imgData, 'JPEG', 0, position, imgWidth, imgHeight);
            heightLeft -= pageHeight;
        }
      ...
      <div class="report-container">
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .print-page {
                width: 210mm;
                height: 296mm; /* أقل بملم واحد لتجنب إنشاء صفحة فارغة إضافية */
                padding: 15mm;
                position: relative;
                overflow: hidden;
                page-break-after: always;
                border-bottom: 1px dashed #ddd; /* خط وهمي لفصل الصفحات في المعاينة */
            }
...
// --- دوال مساعدة ---
// تقسيم مصفوفة إلى أجزاء صغيرة (Pagination Helper)
    ...
    return studentChunks.map((chunk, pageIndex) => {
        ...
        return `
            <div class="print-page">
                ...
                <!-- الجدول يتم تكراره بالكامل في كل صفحة -->
                <table>
                    <thead>
                        <tr>
                            <th width="50">#</th>
...
// مولد تقرير الغياب اليومي (مع التقسيم لصفحات)
...
export const generateGradeDailyReportHTML = (
    ...
    // هنا نقوم بتكرار عملية التقسيم لكل فصل دراسي
...
export const generateStudentReportHTML = (
  ...
  // تقرير الطالب عادة لا يحتاج تقسيم لأنه قصير، لكن سنطبق نفس المنطق للتوحيد
  const ROWS = 15; // عدد أقل بسبب وجود تفاصيل إضافية في الرأس
  const recordChunks = records.length > 0 ? chunkArray(records, ROWS) : [[]];
  ...
  return recordChunks.map((chunk, pageIndex) => {
      ...
      : chunk.map((record, index) => {
        const globalIndex = (pageIndex * ROWS) + index + 1;
